...
(function ($, T) {
    //初始化 分类
    T.getJson("/m/album/groups", {
        state: 1
    }, function (json) {
        var list = json.data.list;
        if (json.state && list && list.length > 0) {
            var $sels = $("select[name='groupId']");
            var html = "";
    ...
    $("#fileUp").uploadify({
        ...
        'uploader': '/m/album/uploadImage',
        'formData': {albumId: $('input[name="albumId"]').val()},
        'buttonText': '选择文件',
    ...
    var getList = function (page) {
        ...
        T.getJson("/m/album/list", data, function (json) {
            if (json.state) {
    ...
    var loadItem = function (id) {
        ...
        T.getJson("/m/album/item", {
            albumId: id
        }, function (json) {
            if (!json.state) {
                T.msg(json.msg || "获取相册信息失败！");
                return false;
            }
            $(".m-panel-title li").removeClass("active");
            $(".m-panel-title ul").append("<li class=\"m-panel-add active\">编辑相册</li>");
            $(".j-back").show();
    ...
    $(document)
        ...
        .delegate(".j-submit", "click", function () {
            ...
            T.getJson("/m/album/add", formData, function (json) {
                if (json.state) {
        ...
        .delegate(".j-delete", "click", function () {
            var id = $(this).parents("td").data("id");
            if (!confirm("确认删除该相册？")) return false;
            T.getJson("/m/album/delete", {
                packageId: id
            }, function (json) {
                if (json.state) {
                    T.msg(json.msg || "删除成功！");
        ...
        .delegate(".j-restore", "click", function () {
            var id = $(this).parents("td").data("id");
            if (!confirm("确认还原该相册？")) return false;
            T.getJson("/m/album/restore", {
                packageId: id
            }, function (json) {
                if (json.state) {
                    T.msg(json.msg || "还原成功！");
