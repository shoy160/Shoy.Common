...
namespace Shoy.Wpf.Core
{
    ...
    public class Messenger
    {
        ...
        private void VerifyParameterType(string message, Type parameterType)
        {
            ...
            if (_messageToActionsMap.TryGetParameterType(message, out previouslyRegisteredParameterType))
            {
                ...
                else
                {
                    ...
                    {
                        throw new TargetParameterCountException(Format(
        ...
        public void Notify(string message, object parameter)
        {
            ...
            if (_messageToActionsMap.TryGetParameterType(message, out registeredParameterType))
            {
                if (registeredParameterType == null)
                    throw new TargetParameterCountException(Format("Cannot pass a parameter with message '{0}'. Registered action(s) expect no parameter.", message));
            }
        ...
        public void Notify(string message)
        {
            ...
            {
                if (registeredParameterType != null)
                    throw new TargetParameterCountException(Format("Must pass a parameter of type {0} with this message. Registered action(s) expect it.", registeredParameterType.FullName));
            }
        ...
        public static Messenger Default
            => Singleton<Messenger>.Instance ?? (Singleton<Messenger>.Instance = new Messenger());
    }
}
